...
- **Document support**: extracts text from **`.docx`** (paragraphs *and* tables),
  including files currently open in Word / synced by OneDrive.
...
**Supported input for extraction:** `.docx` (Word 2007+). Plain `.txt` / `.md` are
read as-is. For PDF / legacy `.doc` / `.odt` / `.rtf`, convert to `.docx` first.

### CLI runner (any OS)

```bash
# .docx -> text, then score:
